...
function FindSchemes() {
  ...
  const [formData, setFormData] = useState({
    ...
    age: "",
    gender: "",
    state: "Maharashtra",          // Fixed for Maharashtra only
    district: "",
    marital_status: "",
  ...
  const getTotalQuestions = () => {
    if (formData.occupation === "Student") return 18;
    if (formData.occupation === "Farmer") return 18;
    if (formData.occupation === "Business Owner / Self-employed") return 18;
    if (formData.occupation === "Homemaker") return 17;
    return 13;
  };
  ...
  const validateCurrentStep = () => {
    switch (step) {
      ...
      case 5: return formData.marital_status !== "";
      case 6: return formData.disability_status !== null;
      case 7: return formData.annual_income !== "";
      case 8: return formData.is_bpl !== null;
      case 9: return formData.rural_urban !== "";
      case 10: return formData.caste_category !== "";
      case 11: return formData.loan_required !== null;
      case 12: return formData.house_ownership !== null;
      case 13: return formData.occupation !== "";
      default: return true;
    }
  };
  ...
  const validateConditionalStep = () => {
    if (formData.occupation === "Student") {
      if (step === 14) return formData.education_level !== "";
      if (step === 15) return formData.course !== "";
      if (step === 16) return formData.year_of_study !== "";
      if (step === 17) return formData.institution_type !== "";
      if (step === 18) return formData.current_scholarship !== null;
    }

    if (formData.occupation === "Farmer") {
      if (step === 14) return formData.land_owner !== null;
      if (step === 15) return formData.land_holding !== "";
      if (step === 16) return formData.farming_type !== "";
      if (step === 17) return formData.irrigation_available !== null;
      if (step === 18) return formData.agricultural_loan !== null;
    }

    if (formData.occupation === "Business Owner / Self-employed") {
      if (step === 14) return formData.business_type !== "";
      if (step === 15) return formData.business_registered !== null;
      if (step === 16) return formData.business_age !== "";
      if (step === 17) return formData.business_turnover !== "";
      if (step === 18) return formData.business_financial_need !== null;
    }

    if (formData.occupation === "Homemaker") {
      if (step === 14) return formData.education_level !== "";
      if (step === 15) return formData.skills !== "";
      if (step === 16) return formData.home_based_business !== null;
      if (step === 17) return formData.business_interest !== null;
    }
  ...
  const nextStep = () => {
    ...
    if (step === 17 && formData.occupation === "Homemaker") {
      setStep(19);
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result?.status === "success") {
        ...
        // Update localStorage + notify Navbar immediately
        if (freshProfile?.status === "success" && freshProfile?.data) {
          localStorage.setItem("user", JSON.stringify(freshProfile.data));
          window.dispatchEvent(
            new CustomEvent("userUpdated", { detail: freshProfile.data })
          );
        }
        ...
        navigate("/recommendation-start", {
          replace: true,
          state: { profile: freshProfile?.data },
        });
      } else {
        alert(result?.error || result?.message || "Failed to save profile.");
      }
    } catch (error) {
  ...
  return (
    <div style={styles.container}>
      <h1 style={styles.heading}>Find Government Schemes</h1>
      ...
      <p style={styles.progressText}>
        {step === 19 ? "Review your answers" : `Question ${step} of ${totalQuestions}`}
      </p>
      ...
      {formData.occupation === "Student" && step === 14 && (
      ...
      {formData.occupation === "Student" && step === 15 && (
      ...
      {formData.occupation === "Student" && step === 16 && (
      ...
      {formData.occupation === "Student" && step === 17 && (
      ...
      {formData.occupation === "Student" && step === 18 && (
        <Question title="Are you receiving any scholarship?">
          <YesNo
            value={formData.current_scholarship}
            onChange={(value) => updateField("current_scholarship", value)}
          />
        </Question>
      )}
      ...
      {formData.occupation === "Farmer" && step === 14 && (
      ...
      {formData.occupation === "Farmer" && step === 15 && (
        <Question title="How much agricultural land do you own? (in acres)">
          <input
            type="number"
            min="0"
            step="0.1"
      ...
      {formData.occupation === "Farmer" && step === 16 && (
      ...
      {formData.occupation === "Farmer" && step === 17 && (
        <Question title="Do you have access to irrigation?">
          <YesNo
            value={formData.irrigation_available}
            onChange={(value) => updateField("irrigation_available", value)}
          />
        </Question>
      )}

      {formData.occupation === "Farmer" && step === 18 && (
      ...
      {formData.occupation === "Business Owner / Self-employed" && step === 14 && (
      ...
      {formData.occupation === "Business Owner / Self-employed" && step === 15 && (
        <Question title="Is your business registered?">
          <YesNo
            value={formData.business_registered}
            onChange={(value) => updateField("business_registered", value)}
          />
        </Question>
      )}

      {formData.occupation === "Business Owner / Self-employed" && step === 16 && (
        <Question title="How long has your business been operating? (in years)">
          <input
            type="number"
            min="0"
      ...
      {formData.occupation === "Business Owner / Self-employed" && step === 17 && (
        <Question title="What is your annual business turnover?">
          <input
            type="number"
            min="0"
            value={formData.business_turnover}
            onChange={(e) => updateField("business_turnover", e.target.value)}
      ...
      {formData.occupation === "Business Owner / Self-employed" && step === 18 && (
        <Question title="Are you looking for business financial assistance?">
          <YesNo
            value={formData.business_financial_need}
            onChange={(value) => updateField("business_financial_need", value)}
          />
        </Question>
      )}
      ...
      {formData.occupation === "Homemaker" && step === 14 && (
      ...
      {formData.occupation === "Homemaker" && step === 15 && (
      ...
      {formData.occupation === "Homemaker" && step === 16 && (
        <Question title="Do you run a home-based business or activity?">
          <YesNo
            value={formData.home_based_business}
            onChange={(value) => updateField("home_based_business", value)}
          />
        </Question>
      )}

      {formData.occupation === "Homemaker" && step === 17 && (
...
function Question({ title, children }) {
  return (
    <div>
      <h2 style={styles.question}>{title}</h2>
      {children}
    </div>
  );
}
...
function Review({ formData, onSubmit, submitting }) {
  ...
  return (
    <div>
      <h2 style={styles.question}>Review your answers</h2>
      ...
      <div style={styles.reviewBox}>
        ...
        <p><strong>Disability:</strong> {displayBoolean(formData.disability_status)}</p>
        <p><strong>Annual Income:</strong> ₹{formData.annual_income}</p>
        <p><strong>BPL:</strong> {displayBoolean(formData.is_bpl)}</p>
        <p><strong>Area:</strong> {formData.rural_urban}</p>
        <p><strong>Caste:</strong> {formData.caste_category}</p>
        <p><strong>Loan Required:</strong> {displayBoolean(formData.loan_required)}</p>
        <p><strong>House Ownership:</strong> {displayBoolean(formData.house_ownership)}</p>
        <p><strong>Occupation:</strong> {formData.occupation}</p>

        {formData.occupation === "Student" && (
          <>
            <p><strong>Education:</strong> {formData.education_level}</p>
            <p><strong>Course:</strong> {formData.course}</p>
            <p><strong>Year of Study:</strong> {formData.year_of_study}</p>
            <p><strong>Institution:</strong> {formData.institution_type}</p>
            <p><strong>Current Scholarship:</strong> {displayBoolean(formData.current_scholarship)}</p>
          </>
        )}

        {formData.occupation === "Farmer" && (
          <>
            <p><strong>Land Owner:</strong> {displayBoolean(formData.land_owner)}</p>
            <p><strong>Land Holding:</strong> {formData.land_holding} acres</p>
            <p><strong>Farming Type:</strong> {formData.farming_type}</p>
            <p><strong>Irrigation:</strong> {displayBoolean(formData.irrigation_available)}</p>
            <p><strong>Agricultural Loan:</strong> {displayBoolean(formData.agricultural_loan)}</p>
          </>
        )}
        ...
        {formData.occupation === "Business Owner / Self-employed" && (
          <>
            ...
            <p><strong>Business Registered:</strong> {displayBoolean(formData.business_registered)}</p>
            <p><strong>Business Age:</strong> {formData.business_age} years</p>
            <p><strong>Business Turnover:</strong> ₹{formData.business_turnover}</p>
            <p><strong>Financial Assistance:</strong> {displayBoolean(formData.business_financial_need)}</p>
          </>
        )}
        ...
        {formData.occupation === "Homemaker" && (
          <>
            ...
            <p><strong>Home Based Business:</strong> {displayBoolean(formData.home_based_business)}</p>
            <p><strong>Business Interest:</strong> {displayBoolean(formData.business_interest)}</p>
          </>
        )}
      </div>
